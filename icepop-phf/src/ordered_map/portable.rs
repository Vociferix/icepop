...
mod tests {
    ...
    fn map_of(entries: impl IntoIterator<Item = (u32, u32)>) -> PortableOrderedMap<u32, u32> {
        let mut builder =
    ...
    fn every_constructor_reaches_the_same_builder() {
        assert!(PortableOrderedMap::<u32, u32>::builder().build().is_empty());
        assert!(PortableOrderedMap::<u32, u32>::builder_with_capacity(8).capacity() >= 8);
        assert_eq!(
            PortableOrderedMap::<u32, u32>::builder_with_hasher(DefaultHasherSeed::with_seed(3))
                .hasher()
                .seed(),
            3,
        );
        assert!(
            PortableOrderedMap::<u32, u32>::builder_with_capacity_and_hasher(
                8,
        ...
        assert!(PortableOrderedMap::<u32, u32>::default().is_empty());
        assert!(
    ...
    fn the_builder_inserts_updates_and_removes() {
        let mut builder = PortableOrderedMap::<u32, u32>::builder();
    ...
    fn collecting_keeps_the_last_of_two_equal_keys() {
        ...
        let map: PortableOrderedMap<u32, u32> =
            [(1u32, 10u32), (2, 20), (1, 11)].into_iter().collect();
