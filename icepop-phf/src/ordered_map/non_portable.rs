//! Lookup and build interface for an [`OrderedMap`] parameterized by [`NonPortable`].
...
impl<K, V> OrderedMap<K, V, DefaultHasherSeed, NonPortable> {
...
impl<K, V, S> OrderedMap<K, V, S, NonPortable>
where
    S: BuildHasher,
{
    ...
    ///
    /// let builder = OrderedMap::<&str, u32>::builder_with_hasher(DefaultHasherSeed::with_seed(7));
    ///
    ...
    /// use icepop_phf::{DefaultHasherSeed, OrderedMap};
    ///
    /// let builder = OrderedMap::<&str, u32>::builder_with_capacity_and_hasher(
    ///     10,
...
impl<K, V, S> Default for OrderedMap<K, V, S, NonPortable>
where
    S: Default,
{
    fn default() -> Self {
...
impl<K, V, S> FromIterator<(K, V)> for OrderedMap<K, V, S, NonPortable>
where
...
impl<K, V, S> PartialEq for OrderedMap<K, V, S, NonPortable>
where
    K: PartialEq,
    V: PartialEq,
{
...
impl<K, V, S> Eq for OrderedMap<K, V, S, NonPortable>
where
    K: Eq,
    V: Eq,
{
...
impl<K, V, S> Builder<K, V, S, NonPortable> {
    ...
    ///
    /// ```
    /// use icepop_phf::{DefaultHasherSeed, ordered_map::Builder};
    ///
    ...
    S: BuildHasher,
{
    ...
    /// assert_eq!(map.get("a"), Some(&1));
    /// ```
    pub fn build(self) -> OrderedMap<K, V, S, NonPortable> {
        OrderedMap {
...
mod tests {
    ...
    fn map_of(entries: impl IntoIterator<Item = (u32, u32)>) -> OrderedMap<u32, u32> {
        let mut builder =
    ...
    fn every_constructor_reaches_the_same_builder() {
        ...
        assert!(OrderedMap::<u32, u32>::builder_with_capacity(8).capacity() >= 8);
        assert_eq!(
            OrderedMap::<u32, u32>::builder_with_hasher(DefaultHasherSeed::with_seed(3))
                .hasher()
                .seed(),
            3,
        );
        assert!(
            OrderedMap::<u32, u32>::builder_with_capacity_and_hasher(
                8,
        ...
        assert!(OrderedMap::<u32, u32>::default().is_empty());
    ...
    fn the_builder_inserts_updates_and_removes() {
        let mut builder = OrderedMap::<u32, u32>::builder();
        ...
        *builder.get_key_value_mut(&1u32).unwrap().1 += 1;
        ...
        // Removal closes the gap rather than reordering what is left.
        assert_eq!(builder.keys().copied().collect::<Vec<_>>(), [1, 2, 3]);
    }

    #[test]
    fn collecting_keeps_the_last_of_two_equal_keys() {
        // `insert` semantics, matching the standard library's `HashMap`.
        let map: OrderedMap<u32, u32> = [(1u32, 10u32), (2, 20), (1, 11)].into_iter().collect();
        assert_eq!(map.get(&1u32), Some(&11));
        // The replaced entry keeps the position it was first inserted at.
        assert_eq!(map.as_slice(), &[(1, 11), (2, 20)]);
    ...
    fn equality_compares_keys_and_values() {
        ...
        assert_eq!(a, map_of((0..8).map(|k| (k, k * 10))));
        // Entry order is part of the value here, so the reversed build is a different map.
        assert_ne!(a, map_of((0..8).rev().map(|k| (k, k * 10))));
