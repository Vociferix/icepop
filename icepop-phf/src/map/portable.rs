//! Lookup and build interface for a [`Map`] parameterized by [`Portable`].
...
impl<K, V> Map<K, V, DefaultHasherSeed, Portable> {
    ...
    ///
    /// let mut builder = PortableMap::<&str, u32>::builder();
    /// builder.insert("a", 1);
    ///
    ...
    /// use icepop_phf::PortableMap;
    ///
    /// let builder = PortableMap::<&str, u32>::builder_with_capacity(10);
    ///
    ...
    S: PortableBuildHasher,
{
    ...
    ///
    /// ```
    /// use icepop_phf::{DefaultHasherSeed, PortableMap};
    ///
    /// let builder = PortableMap::<&str, u32>::builder_with_hasher(DefaultHasherSeed::with_seed(7));
    ///
    ...
    /// use icepop_phf::{DefaultHasherSeed, PortableMap};
    ///
    /// let builder = PortableMap::<&str, u32>::builder_with_capacity_and_hasher(
    ///     10,
    ...
    /// use icepop_phf::PortableMap;
    ///
    /// let map: PortableMap<&str, u32> = [("a", 1u32), ("b", 2)].into_iter().collect();
    ///
    ...
    /// use icepop_phf::PortableMap;
    ///
    /// let mut map: PortableMap<&str, u32> = [("a", 1u32)].into_iter().collect();
    ///
    ...
    /// use icepop_phf::PortableMap;
    ///
    /// let mut map: PortableMap<&str, u32> = [("a", 1u32), ("b", 2)].into_iter().collect();
    ///
...
impl<K, V, S> Default for Map<K, V, S, Portable>
where
    S: Default,
{
    fn default() -> Self {
...
impl<K, V, S> FromIterator<(K, V)> for Map<K, V, S, Portable>
where
...
impl<K, V, S> PartialEq for Map<K, V, S, Portable>
where
    K: PortableHash + PortableEq + PartialEq,
    V: PartialEq,
    S: PortableBuildHasher,
{
    fn eq(&self, other: &Self) -> bool {
        ...
        self.iter().all(|(key, value)| {
            other
                .get_key_value(key)
                .is_some_and(|(other_key, other_value)| key == other_key && value == other_value)
        })
    }
}

impl<K, V, S> Eq for Map<K, V, S, Portable>
where
    K: PortableHash + PortableEq + Eq,
    V: Eq,
    S: PortableBuildHasher,
{
}
...
impl<K, V, S> Builder<K, V, S, Portable> {
    ...
    ///
    /// ```
    /// use icepop_phf::{DefaultHasherSeed, map::PortableBuilder};
    ///
    ...
    S: PortableBuildHasher,
{
    ...
    /// assert_eq!(map.get("a"), Some(&1));
    /// ```
    pub fn build(self) -> Map<K, V, S, Portable> {
        Map {
...
cfg_select!(feature = "serde" => {
    impl<K, V, S> serde::Serialize for Map<K, V, S, Portable>
    where
        K: serde::Serialize,
        V: serde::Serialize,
        S: serde::Serialize,
    ...
    impl<'de, K, V, S> serde::Deserialize<'de> for Map<K, V, S, Portable>
    where
...
mod tests {
    use super::*;
    use crate::PortableMap;

    use alloc::vec::Vec;

    fn map_of(entries: impl IntoIterator<Item = (u32, u32)>) -> PortableMap<u32, u32> {
        let mut builder =
            PortableMap::<u32, u32>::builder_with_hasher(DefaultHasherSeed::with_seed(1));
        for (k, v) in entries {
            builder.insert(k, v);
        }
        builder.build()
    ...
    fn every_constructor_reaches_the_same_builder() {
        assert!(PortableMap::<u32, u32>::builder().build().is_empty());
        assert!(PortableMap::<u32, u32>::builder_with_capacity(8).capacity() >= 8);
        assert_eq!(
            PortableMap::<u32, u32>::builder_with_hasher(DefaultHasherSeed::with_seed(3))
                .hasher()
                .seed(),
            3,
        );
        assert!(
            PortableMap::<u32, u32>::builder_with_capacity_and_hasher(
                8,
        ...
        assert!(
            Builder::<u32, u32, DefaultHasherSeed, Portable>::new()
                .build()
                .is_empty()
        );
        assert!(Builder::<u32, u32, DefaultHasherSeed, Portable>::with_capacity(8).capacity() >= 8);
        assert_eq!(
            Builder::<u32, u32, DefaultHasherSeed, Portable>::with_hasher(
                DefaultHasherSeed::with_seed(3)
            )
            .hasher()
            .seed(),
        ...
        assert!(
            Builder::<u32, u32, DefaultHasherSeed, Portable>::with_capacity_and_hasher(
                8,
                DefaultHasherSeed::with_seed(3)
            )
            .capacity()
        ...
        assert!(PortableMap::<u32, u32>::default().is_empty());
        assert!(
            Builder::<u32, u32, DefaultHasherSeed, Portable>::default()
                .build()
                .is_empty()
        );
    }
    ...
    fn the_builder_inserts_updates_and_removes() {
        let mut builder = PortableMap::<u32, u32>::builder();
    ...
    fn collecting_keeps_the_last_of_two_equal_keys() {
        ...
        let map: PortableMap<u32, u32> = [(1u32, 10u32), (2, 20), (1, 11)].into_iter().collect();
        assert_eq!(map.get(&1u32), Some(&11));
        assert_eq!(map.len(), 2);

        let mut builder: Builder<u32, u32, DefaultHasherSeed, Portable> =
    ...
    fn equality_compares_keys_and_values() {
        ...
        let mut ba: Builder<u32, u32, DefaultHasherSeed, Portable> =
            [(1u32, 10u32)].into_iter().collect();
        let bb: Builder<u32, u32, DefaultHasherSeed, Portable> =
            [(1u32, 10u32)].into_iter().collect();
        assert_eq!(ba, bb);
        ba.insert(1, 11);
        assert_ne!(ba, bb);
