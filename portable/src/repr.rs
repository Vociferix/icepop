...
/// A borrowed field within a generated representation.
///
/// Representations generated by `#[portable(repr)]` hold one of these per field. It erases the
/// lifetime of the borrow so that a composite representation can be named by
/// [`VisitPortableRepr::Repr`], which has no lifetime parameter, and hands the borrow back out
/// back out through [`Deref`](core::ops::Deref), so a representation's fields are
/// reached exactly as the original type's are.
///
/// # Invariant
///
/// A `Field` must not outlive the value it borrows. There is deliberately no way to construct
/// one through this crate's public API: they are built only inside
/// [`visit_portable_repr`](VisitPortableRepr::visit_portable_repr) by the generated code, which
/// lends the representation out by reference for the duration of a single call and so cannot
/// leak a `Field` past the borrow it was built from. Because construction is not public,
...
///
/// // `Option`'s representation lends its payload out the same way a generated one does.
/// assert!(Some(1usize).portable_eq(&Some(1u64)));
/// ```
#[repr(transparent)]
pub struct Field<T: ?Sized>(NonNull<T>);

impl<T: ?Sized> core::ops::Deref for Field<T> {
    type Target = T;

    fn deref(&self) -> &T {
        // SAFETY: a `Field` is only ever built by generated code, from a borrow that outlives
        // the call the representation is lent out for, so the pointee is still live. The
        // returned borrow is tied to `&self`, so it cannot outlive it either.
